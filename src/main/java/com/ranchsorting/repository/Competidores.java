...
public class Competidores implements Serializable {
	...
	/*
	 * @SuppressWarnings("unchecked") public List<Competidor>
	 * filtrados(CompetidorFilter filtro) {
	 * 
	 * Session session = manager.unwrap(Session.class); Criteria criteria =
	 * session.createCriteria(Competidor.class);
	 * 
	 ...
	 * criteria.add(Restrictions.ge("dataNascimento",
	 * filtro.getDtNascimentoDe())); }
	 * 
	 * if (filtro.getDtNascimentoAte() != null) {
	 * criteria.add(Restrictions.le("dataNascimento",
	 * filtro.getDtNascimentoAte())); }
	 * 
	 * if (StringUtils.isNotBlank(filtro.getResponsavel())) {
	 * criteria.add(Restrictions.ilike("responsavel", filtro.getResponsavel(),
	 * MatchMode.ANYWHERE)); }
	 * 
	 * if (StringUtils.isNotBlank(filtro.getContato())) {
	 * criteria.add(Restrictions.ilike("contato", filtro.getContato(),
	 * MatchMode.ANYWHERE)); }
	 * 
